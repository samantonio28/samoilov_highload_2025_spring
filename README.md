...
3) В зависимости от вида трафика сделать следующее:
    ...
    - если мобильный трафик (а он преимущественный), то LTM балансирует трафик на nginx, который в свою очередь балансирует трафик на мобильный кластер по стратегии weighted round-robin
...
Kubernetes, разработанный Google, - открытый контейнерный оркестратор, который позволяет развертывать и масштабировать приложения в кластере. Он позволяет автоматизировать процессы развертывания, обновления и отказоустойчивости приложений.
...
$
464 476.24 \cdot 0.2 = 92895.248 \,\,\text{RPS} \\
2267 \cdot 0.2 = 441.8 \,\,\text{Гбит/сек} \\
$
...
- Проводить e2e health check-и каждые 30 минут + availability check-и каждую минуту + functional check-и каждые 5 минут + проверки базы данных и кеша каждые 5 минут
...
| avatar_id         | int64            |      8       |          8           |
| geo_position      | numeric(8,5) * 2 |      44      |          44          |
| **Сумма**         |                  |   **1236**   |         **424**      |
...
Допустим, что кука выдается на 3 суток. При размере суточной аудитории в 42 млн пользователей получим общий объём таблицы: 6.9 Гб
...
Размер одной записи - около 38 байт.

В Tinder ecть 11 видов интересов. Размер таблицы - 418 байт.
