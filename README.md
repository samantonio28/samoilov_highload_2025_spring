...
| DynamoDB       | БД для данных пользователей                     | Масштабируемость, высокая скорость, гибкость     | 
| Redis         | БД для временного хранения сообщений и сессий            | Масштабируемость, высокая скорость               |
| Prometheus    | Протокол для работы с метриками                        | Стандарт индустрии                               |
| TSDB    | Сбор метрик, их хранение                        | Специализированная база данных временных рядов                               |
| Grafana       | Визуализация собранных метрик                   | Лидер индустрии                                  |
| Amazon S3            | Хранилище пользовательских файлов               | Уменьшение задержки и исходящего трафика         |
...
Go быстрее и эффективнее Python, особенно для задач, связанных с CPU. Go предоставляет легковесные горутины и каналы, что облегчает параллельную обработку задач и асинхронную программирование без особой сложности. Также Python однопоточный в целом, это сильно ограничивает скорость.
...
Amazon DynamoDB идеально подходит для хранения данных, которые имеют переменную структуру включающих в себя ассоциативные массивы, что характерно для профиля пользователя (согласно [источнику](https://eathealthy365.com/tinder-s-database-model-explained-from-nosql-to-scale/))

### БД для хранения свайпов, совпадений и чатов

DynamoBD в связке с Redis подходят для кеширования сообщений, "горячих" пользователей и свайпов. Для аналитики и поиска подойдет Elasticsearch. 

### БД для временного хранения сообщений и сессий

Redis подходит для данных, к которым требуется высокая скорость доступа. Также если Redis упадет, то потерянные данные будут не критичными и для их восстановления пользователям достаточно опять авторизоваться и зайти в чаты.
